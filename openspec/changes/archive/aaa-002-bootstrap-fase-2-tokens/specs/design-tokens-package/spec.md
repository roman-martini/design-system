...
- **THEN** los campos `name`, `version`, `description`, `author`, `license`, `repository.directory`, `engines`, `publishConfig.access` SHALL estar presentes y no vacíos
...
- **WHEN** se ejecuta `pnpm -F @romanmartinidev/tokens pack --dry-run`
...
### Requirement: Jerarquía semantic.border respetada (fix de auditoría)
...
Esta requirement responde a un finding de la auditoría: en el estado inicial pre-Fase 2, `border.subtle` y `border.default` apuntaban ambos a `{color.neutral.100}` en light, rompiendo la jerarquía. Esta requirement queda como contrato testable post-fix.
...
El package SHALL exponer un token `shadow.focus` (primitive) y su uso semántico vía `semantic.shadow.focus` (o equivalente). Este token define el box-shadow que aplica un focus ring visible para cumplir [WCAG 2.4.7 Focus Visible](https://www.w3.org/WAI/WCAG21/Understanding/focus-visible.html).

#### Scenario: token shadow.focus existe en primitives

- **WHEN** se inspecciona `packages/tokens/src/primitives/shadow.json`
- **THEN** existe una key `focus` con un value de tipo box-shadow CSS válido (ej. `0 0 0 3px rgba(...)`)

#### Scenario: build emite la variable

- **WHEN** se ejecuta `pnpm -F @romanmartinidev/tokens build`
- **THEN** `dist/tokens.css` SHALL contener `--ds-shadow-focus` con un valor de box-shadow no vacío
