...
`DsButton` cubre acción, variantes, sizes y disabled accesible, pero no el estado más común de un botón que dispara trabajo asíncrono: **en curso**. Hoy el consumidor cablea a mano un `ds-spinner` embebido + una bandera de "guardando" + una guarda contra el doble submit, en cada pantalla. [HU-017](../../../docs/product/epics/EP-002-kit-componentes/HU-017-button-loading.md) fue refinada con el PO el 2026-07-22 (precedencia `loading` sobre `disabled`, contenido configurable con default seguro, anuncio por `aria-busy`) y cierra ese hueco en el propio componente. Respalda la **prioridad 1** del repo (buenas prácticas: a11y y prevención de doble submit de serie) y ejecuta [D-007](../../../docs/product/decisiones.md) (a11y como feature).
...
- **Bloqueo accesible**: con `loading=true` la guarda del manejador impide disparar `clicked` (click + Enter/Space), pero el botón **permanece focuseable y anunciado** — no usa `disabled` nativo (criterio de a11y de [ADR-011](../../../docs/architecture/adr/ADR-011-estado-disabled-accesible.md)).
...
- Changeset: **minor** de components (input aditivo + refinamiento visual; la API previa no cambia). El par se versiona lockstep ([ADR-015](../../../docs/architecture/adr/ADR-015-versionado-lockstep.md)) al release.
...
1. **Reutilizar el `disabled` nativo para bloquear durante la carga** — descartada: reintroduce los tres agujeros de a11y que [ADR-011](../../../docs/architecture/adr/ADR-011-estado-disabled-accesible.md) resolvió (fuera del tab order, sin anuncio, sin comunicar el estado) y no distingue "no disponible" de "en curso". `aria-busy` + guarda mantiene el botón descubrible y comunica el estado correcto.
...
- **Dependencias**: ninguna nueva. Reutiliza `DsSpinner` (composición interna) y el patrón de guarda de [ADR-011](../../../docs/architecture/adr/ADR-011-estado-disabled-accesible.md).
...
- **Sin ADR previsto**: `loading` hoy solo lo adopta `DsButton`. Por la misma lógica de [ADR-011](../../../docs/architecture/adr/ADR-011-estado-disabled-accesible.md) ("sin primitiva compartida hasta el segundo consumidor"), un patrón transversal de loading se decidiría por ADR recién si un segundo componente de acción lo adopta.
