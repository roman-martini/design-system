# Delta — monorepo-structure

Define los requisitos estructurales mínimos del monorepo `agent-design-sistem` que esta Fase 1 establece.
...
### Requirement: angular-app/ no existe post-Fase 1

La carpeta `angular-app/` en root SHALL ser eliminada en Fase 1. La app de prueba SHALL regenerarse en Fase 4 en `apps/playground/` con Angular 21 desde cero.

#### Scenario: ningún workspace conflictúa con apps/playground futuro

- **GIVEN** post-Fase 1
- **WHEN** se inspecciona el repo
- **THEN** no existe `angular-app/` en ningún nivel y `apps/` está vacío o no existe (se creará en Fase 4)
