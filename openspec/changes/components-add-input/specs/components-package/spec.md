...
#### Scenario: two-way binding e integración CVA (CA-005.1)
...
#### Scenario: anatomía accesible del field (CA-005.2)
...
#### Scenario: tipos de texto en lista blanca (CA-005.3)
...
#### Scenario: estado invalid automático desde el NgControl (CA-005.4)
...
#### Scenario: override manual del estado invalid (CA-005.4)
...
#### Scenario: disabled nativo vía forms API (CA-005.5)
...
#### Scenario: estilos y contraste por tokens (CA-005.6)
...
- **AND** los pares texto/placeholder/borde sobre el bg del field SHALL cumplir AA calculados por script en los 4 themes (`text-placeholder` referencia `text.secondary` tras el fix de este change)

#### Scenario: slots pasivos prefix y suffix (CA-005.7)
...
#### Scenario: foco visible en el field (CA-005.8)
