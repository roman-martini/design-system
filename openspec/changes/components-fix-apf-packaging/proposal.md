...
- **Changelog y procedencia**: `@changesets/changelog-github` para que las entradas enlacen commit/PR/autor [release-npm-09]; `id-token: write` + `publishConfig.provenance` preparados sin publicar nada, según [D-018](../../../docs/product/decisiones.md)(c) [release-npm-05].
- Changesets: **patch** de components y de tokens (lockstep [ADR-015](../../../docs/architecture/adr/ADR-015-versionado-lockstep.md)) — el fix de compilación cambia el artefacto emitido, no la API.
...
Decisión del PO en sesión (2026-07-27), sobre las tres opciones de [release-npm-07]:
...
- **Veto npm**: **levantado por el PO el 2026-07-27** ([D-028](../../../docs/product/decisiones.md)). Este change **no publica**: el destino acordado es un **0.3.0** con los changesets acumulados, y el pipeline sigue bloqueado por [ci-cd-01] hasta la Parte E.
