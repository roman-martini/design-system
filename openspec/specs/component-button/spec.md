...
Contrato de tests de `DsButton` con Vitest: creación del componente, render del `<button>` y comportamiento del output `clicked` con y sin `disabled`. El resto del contrato del botón (disabled accesible, iconografía) es transversal y vive en `components-package`.
