...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **H1 — Kit mínimo viable para una app real** | Tanda 1 ([D-009](decisiones.md)) completa + tanda 2 ([D-011](decisiones.md)): HU-012…HU-016 (EP-002)  | Una app real se construye 100% con componentes del DS             | **Cumplido** (2026-08-03) — el prototipo de [D-023](decisiones.md) está en `/prototipo` del playground: el formulario de la referencia `modern-minimal` construido con el kit, sin replicar estilos de componente. Dejó 3 gaps registrados en el [BACKLOG](../backlog/BACKLOG.md), ninguno bloqueante |
...
EP-002: fixes de componentes       → Parte G de la review COMPLETA 7/7 (aaa-045…aaa-051):
                                    menú, select, toast, modal, button, avatar y
                                    checkbox-radio. Tres convenciones implícitas pasaron
                                    a requirement con test; tres componentes tenían
                                    tokens declarados que nadie consumía
EP-001: Parte H (tokens)           → COMPLETA (2026-08-08): sistema (aaa-052) + matriz
                                    brand×scheme (aaa-054) + deuda component.* (aaa-053:
                                    41 tokens retirados, botón semibold conectado a su
                                    capa, hovers y thumb theme-aware; deuda en 0)
...
Última entrega: **`aaa-053` — la deuda de la capa `component.*` saldada**, archivado el 2026-08-08, y **con eso cierra la Parte H**. El principio que queda instalado: **cada token de `component.*` o tiene consumidor o no existe** — la clase de bug "token muerto que un día diverge", que la Parte G encontró tres veces, se queda sin lugar donde vivir. Se retiraron **41 tokens** (los 36 triados por la auditoría del 2026-08-04 más un descubrimiento del apply: `button.link.*` declaraba una variante que `DsButtonVariant` nunca tuvo — no había nada a qué conectarla) y se conectó el resto: el botón consume su capa de color completa (sin cambio visual, pero la divergencia futura queda estructuralmente imposible) y su peso desde el token — **semibold 600, la verdad que el CSS pisaba con 500 en silencio**. El thumb del switch tomó la cadena de superficie invertida de `aaa-051` (blanco fijo → theme-aware), checkbox y radio ganaron el hover en ambos estados y la card `outline`/`elevated` eleva al hover. La re-auditoría verificó el cierre: **0 hardcodes, 0 violaciones, deuda `component.*` en 0** — solo quedan los 6 falsos positivos del detector TS (fix chico pendiente) y 2 `label-font-size` ya congelados en el trinquete tipográfico. En el gate visual el PO además fijó el rumbo siguiente: los botones migran al look de PrimeNG (Aura) como change propio — con la advertencia ya medida de que el indigo de la referencia falla AA (4.46:1) y habrá que resolverlo en tokens, no copiarlo.

Entrega anterior: **`aaa-052` + `aaa-054` — los tokens de sistema y la matriz brand × scheme**, archivados juntos el 2026-08-07 (primer bloque de la Parte H). `aaa-052` hizo que el **anillo de foco componga desde `semantic.color.focus-ring`** — las marcas declaraban su color de foco y el sistema las ignoraba: azul fijo en brand-a y brand-b —, dio **elevación visible en dark** (D-025), alineó los overlays a la escala primitiva y congeló por **trinquete** 28 referencias tipográficas legadas. `aaa-054` nació _durante su gate visual_: el PO detectó que dark + marca rompía el radio card del prototipo H1, y la causa era estructural — **todo token que una marca overridea perdía su variante oscura** por cascada. Los overlays `brand-*-dark` modelan la matriz, y el gate de contraste ahora corre cada par también en los **scopes combinados**: en su primera corrida cazó 2 violaciones que nadie había visto. El mismo gate iteró dos veces el foco de fields con el PO en vivo (ancho del anillo → solo border sobre la cadena `focus-ring`, gateada ≥3:1 en los 6 scopes). Dos lecciones quedaron institucionalizadas: el **criterio de recomendación** del PO entró a CLAUDE.md, y "el gate que evalúa scopes por separado no ve los bugs de combinación" ya no puede repetirse.

Entrega anterior: **`aaa-051` — el theming del checkbox y el radio**, archivado el 2026-08-03, y **con eso cierra la Parte G (7/7)**. El hallazgo heredado decía que el control _desmarcado_ se pintaba blanco en dark; leído el código, eso no pasaba —el CSS ya usaba `bg.surface`, que es theme-aware—. **El defecto real era la marca**: el checkmark llevaba `stroke='white'` fijo dentro de un SVG en data URI y el punto del radio usaba el primitive `--ds-color-white`, justo sobre el fondo que en dark aclara. Medido en Chromium, el contraste pasa de ~3.7:1 a ~4.9:1. No alcanzaba con reemplazar el literal por un `var()`: **un SVG embebido en `url("data:…")` se resuelve como documento aislado y no ve las custom properties**, así que la marca pasó a dibujarse con el SVG como máscara y el color puesto por CSS. Y detrás apareció el patrón que esta parte encontró tres veces —después del botón—: **ninguno de los dos componentes consumía sus tokens**, que llevaban tanto tiempo huérfanos que habían derivado hasta contradecir el render (`radio.dot-color` declaraba el mismo color del fondo marcado, o sea un punto invisible). Ese dato es insumo directo de la auditoría de tokens que sigue: **token huérfano no es sinónimo de inventario deliberado**.

Entrega anterior: **`aaa-050` — el overflow traducible del grupo de avatares**, archivado el 2026-08-03: el sexto de los siete changes de la Parte G, y el más chico. El nombre accesible del "+N" estaba fijo en español y sin forma de cambiarlo — **el único texto del kit anunciado por tecnología asistiva que un consumidor no podía traducir**, cuando `closeLabel`, `dismissLabel`, el `label` del spinner y los seis de `DsPagination` ya lo eran. Se resolvió con un input que **recibe la cantidad oculta**, no con un string con placeholder: el texto entero depende del número, así que traducirlo suele exigir pluralización ("1 more" contra "2 more") o un orden de palabras distinto, y ninguna de las dos cosas se resuelve concatenando fragmentos fijos. Diverge a propósito de la forma que usa `DsPagination`, que compone literales alrededor de un número porque ahí sí alcanza; queda anotado en el design para que la próxima lectura no lo "unifique" por consistencia aparente. Sin cambio visual: el "+N" mide y se ve igual.

Y antes: **`aaa-049` — el dimensionamiento del botón**, archivado el 2026-08-03: **el quinto de los siete changes de la Parte G**, y el único con **cambio visual deliberado**. Salió de un reporte del PO —el texto del botón no se veía centrado— que resultó ser la punta de algo más grande. Medido en Chromium: el `line-height` resolvía a 20 px contra un alto natural de texto de 21, así que la caja de línea lo recortaba y el sobrante negativo, repartido en medio píxel por lado, se iba hacia arriba por redondeo. Pero la causa de fondo era otra: **los tokens `component.button.*` de altura, padding, tamaño de fuente y gap existían desde el bootstrap y nadie los consumía**. El botón se dimensionaba con `semantic.space.*` y primitives sueltos, contra la jerarquía que ADR-003 declara, y por eso **medía 38 px donde su token decía 40 y no alineaba con el select ni con los campos de texto** puestos en la misma fila. Conectarlos llevó los tres tamaños a la escala que el sistema declaraba (32/40/48: el `sm` creció 8.5 px, el `lg` se achicó 8.5) y dejó el texto en 8 px arriba y 8 abajo. En el camino apareció una trampa que conviene recordar: `component.button.radius` decía 6 px mientras el render usaba 8, así que adoptarlo tal cual **habría cambiado la curvatura de todos los botones como efecto colateral de un fix de centrado** — se corrigió el token al valor real. La regla que queda escrita: cuando un token y el render discrepan, primero se decide cuál de los dos es la verdad. El change además cierra `type="submit"` con guarda que también bloquea el envío nativo, y el nombre accesible del botón ícono-only — cuya brecha estaba probada en el propio repo, porque la story `OnIconButton` de tooltip **esquivaba `ds-button`** con un `<button>` nativo y estilos inline para poder nombrarlo.

Y antes: **`aaa-048` — el nombre accesible del modal**, archivado el 2026-08-02: **el cuarto de los siete changes de la Parte G**. Un `DsModal` sin `heading` quedaba **sin nombre accesible**: un lector de pantalla anunciaba "diálogo" y nada más. No es un caso de borde — es el modal de confirmación con contenido proyectado, el uso más común del componente. Lo que hace interesante al hallazgo es que **el kit ya había resuelto esto dos veces**: `DsSelect` y `DsFieldBase` exponen los alias `aria-label`/`aria-labelledby` para exactamente este hueco, y el modal nunca los adoptó. Es la tercera convención implícita que destapa esta parte, después de los índices con exports enumerados y del `display` de los popovers. Con tres vías posibles hubo que fijar precedencia, y se eligió **`aria-labelledby` > `heading` > `aria-label`**: un `labelledby` del consumidor es una instrucción directa y gana, y el `heading` gana sobre `aria-label` porque cuando hay título visible el nombre accesible debe coincidir con él ([WCAG 2.5.3](https://www.w3.org/WAI/WCAG22/Understanding/label-in-name.html)) — preferir un label distinto es el antipatrón que esa regla previene. Se descartó **inventar un nombre por defecto**, que habría hecho pasar a axe: apagaría la señal sin resolver el problema, dejando al consumidor sin enterarse de que su modal no dice de qué es. También se cerró el último hardcode dimensional del CSS ([components-12]), con el mismo token que ya usaba el menú.

Y antes: **`aaa-047` — el anuncio del toast**, archivado el 2026-08-02: **el tercero de los siete changes de la Parte G**, sobre [HU-008](epics/EP-002-kit-componentes/HU-008-toast-notificaciones.md). Un solo defecto, pero de los que no se ven: **el primer toast de la sesión podía no anunciarse en un lector de pantalla** — y el primero suele ser el que más importa, la confirmación de que algo se guardó o el error de que no. El contenedor se creaba recién en el primer `show()` y el rol de live region viajaba en el elemento insertado en ese mismo ciclo; una región que aparece junto con su contenido no se anuncia de forma confiable. El hallazgo proponía crear el contenedor antes, pero con el código delante eso no alcanzaba: el contenedor es un popover, **cerrado computa `display: none` y queda fuera del árbol de accesibilidad**, así que cualquier región alojada ahí adentro seguía apareciendo con el primer toast. El anuncio salió del popover a dos regiones persistentes en el `body` (`polite` y `assertive` —dos, porque la urgencia de una región no se puede mutar de forma confiable—) y el elemento visual dejó de declarar rol para no duplicarlo. La guarda de plataforma **no fue opcional**: mover el acceso al DOM al arranque de un service `providedIn: 'root'` habría roto el renderizado en servidor, que hoy sobrevive solo por accidente. Y el presupuesto de bundle volvió a hablar: el techo de `components` subió a 51.94 kB bajo [D-031](decisiones.md), pero al aplicar la regla apareció que **el margen del 5% ya vale más que un componente entero** (2.47 contra 2.32 kB), justo lo que la política existía para impedir — el gate dejó de detectar la entrada silenciosa de un componente y corregirlo pide una decisión.

Y antes: **`aaa-046` — fixes del select**, archivado el 2026-08-01: **el segundo de los siete changes de la Parte G**, sobre [HU-003](epics/EP-002-kit-componentes/HU-003-select-formularios.md). Cinco fixes, y el que más importa no venía de la review sino del uso: **el control se contraía al elegir una opción de label corto**. Medido en Chromium antes de tocar código —siguiendo la lección de `aaa-045`— resultaron ser dos defectos encadenados: el trigger se dimensionaba por el label seleccionado (133 px con el placeholder, 79 px con "Chile") y el posicionador le clavaba ese ancho al listado, que partía las opciones largas en dos líneas. Se resolvió con un piso tokenizado en el trigger y usando su ancho como **mínimo** del listado, no como medida exacta; dimensionarlo por la opción más larga (lo que hace el `<select>` nativo) quedó evaluado y descartado por costo en bundle, no por criterio. De la review salieron los otros cuatro: `touched` no se marcaba nunca si el usuario tabulaba sin abrir —el contrato de forms era inconsistente con `DsFieldBase`—, faltaba el typeahead que el patrón APG exige y `DsMenu` ya tenía, la separación trigger↔listado estaba hardcodeada en JS mientras submenú y tooltip la leían de un token, y el listbox cerrado generaba caja igual que el menú. **Esa última pasó a requirement de `components-package`** con un test que barre todos los CSS de overlay del kit: el select era el último que la violaba, y la convención ya no depende de que el próximo autor tenga un vecino a mano. El change también agotó los dos techos de bundle de `tokens` —el trinquete de [D-031](decisiones.md) frenó el PR y se recalcularon— y dejó `components` **a 60 B de su techo**: el próximo change de la Parte G va a tener que moverlo.
