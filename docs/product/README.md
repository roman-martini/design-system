...
EP-002: fixes de componentes       → Parte G de la review, 6/7 (aaa-045…aaa-050): menú
                                    (hover-timer, panel cerrado, exports), select
                                    (touched, typeahead, ancho estable), toast (el primer
                                    anuncio ya no se pierde), modal (nombre accesible sin
                                    heading), button (alturas tokenizadas, type submit,
                                    ícono-only) y avatar (overflow traducible). Queda
                                    checkbox-radio
