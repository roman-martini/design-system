...
- **Defecto del artefacto publicado, detectado después (2026-07-26)** — la HU sigue **Hecha**: los cuatro CAs se cumplieron y se verificaron. Pero el `0.2.0` que publicó salió en **full compilation mode**, violando Angular Package Format: rompe a consumidores en otra versión de Angular 21.x y rompería seguro en Angular 22. CA-002.3 no lo detectó porque el proyecto de prueba usaba la misma versión de Angular con que se compiló la lib — el defecto es invisible en ese escenario. Corregido en [aaa-038](../../../../openspec/changes/archive/aaa-038-components-fix-apf-packaging/) junto con el resto del empaquetado (LICENSE ausente en el tarball, `engines` del monorepo publicados, `exports` sin `"./package.json"`), y cubierto de ahí en más por el gate `pnpm verify:packaging`. La versión sana sale como **0.3.0** ([D-028](../../../decisiones.md)).
