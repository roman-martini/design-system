...
- **Mismo caso en el botón** (PO, 2026-08-07, reporte `fix-button`: la selección del texto revela la caja y "no se ve centrado"). Medido con Playwright sobre el build (ya con semibold 600 de `aaa-053`): en `md` (40px) la caja de línea de 21px queda a 9.0/10.0 px de los bordes — 0.5px sobre el centro geométrico, el resto inevitable de repartir un sobrante impar (17px) en flex; en `sm` el centrado da 6.5/6.5 exacto. No hay defecto de layout que un padding pueda corregir: medio píxel es el mínimo error posible y el resto de la percepción es la misma óptica de Inter del caso anterior. La vía de corrección es la misma: **metric overrides** en el `@font-face`, una sola vez para todo el sistema.
