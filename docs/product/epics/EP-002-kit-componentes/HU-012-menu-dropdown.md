...
- **Fixes (2026-08-01)** — [`aaa-045 components-fix-menu`](../../../../openspec/changes/archive/aaa-045-components-fix-menu/) archivado, primer change de la Parte G de la review integral. Tres correcciones sobre CA-012.6 y CA-012.7, ninguna con cambio de API: (1) la apertura de submenú por hover ya no sobrevive al cierre ni al cambio de item —antes robaba el foco o dejaba un panel huérfano—; (2) el panel cerrado deja de generar caja (`display: none` fuera de `:popover-open`), lo que eliminaba las barras de scroll que el PO reportó al reabrir un menú con submenús; (3) el panel acota su alto con `component.menu.panel.max-height`. La superficie pública del componente pasó a exports enumerados. Gate visual del PO OK (2026-08-01).
