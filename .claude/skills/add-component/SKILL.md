...
- **`tasks.md`** — replicar la estructura de 8 fases de `aaa-014`, cada task ≤2h con criterio binario:
  ...
  4. **Componente**: `src/lib/<name>/` — standalone, OnPush, signals (`input()`/`output()`/`model()`), template en `.html` con control flow nativo, CSS tokenizado, `index.ts` + export en `public-api.ts`, build APF verde. La generación de los archivos puede **delegarse a `/ng:create`** (agente `ng-component`): el `ng-stack-profile.md` ya cubre prefijo de selector, motor de estilos y framework de test; lo que el perfil no cubre (naming `Ds<Name>` sin sufijo, CSS solo con `var(--ds-*)`, patrón disabled de ADR-011) lo imponen estas tasks al adaptar el resultado.
  5. **Tests de comportamiento** (`<name>.spec.ts`): un test por scenario del delta; suite completa verde.
  6. **Story + playground**: `<name>.stories.ts` CSF 3 (title `Components/<Name>` sin prefijo Ds), demo en playground, `build-storybook` + test de playground verdes.
  7. **Validación de cierre**: `pnpm openspec validate <change> --strict`, `pnpm lint`, `pnpm format:check`, `pnpm -r build`, `pnpm -r test`, auditoría **`/ng:review`** sobre los archivos del componente nuevo como quality gate (sin hallazgos de severidad alta ni media — "crear y revisar con el mismo rasero"), `npm pack --dry-run` (sin `*.spec.ts`/`*.stories.ts` en el tarball), changeset **minor** de components (+ **patch/minor** de tokens si tocó tokens). Última task: **proponer mensaje de commit y esperar OK**.
  8. **ADR + archive** (si hubo decisión promovible): ADR, decisions-log, mover a `archive/<id>-<name>/`, sincronizar spec base, actualizar próximo ID en `openspec/README.md`, catálogo en `docs/architecture/README.md`, grooming del BACKLOG. Última task: proponer commit del archive y esperar OK.
